...
#iterate through daily_transactions (remember, this is a list of strings currently), and for each transaction, split the string around whatever character you replaced the ;,; artifacts
...
for list in daily_transactions_split:			#keeps lists intact for the final append
  clean_list = [] 												#sets new temp list
  for item in list:												#iterates through and takes off all spaces for items
      clean_list.append(item.strip())
      # print(clean_list)
  transactions_clean.append(clean_list)		#adds them back in as they were (beginning of for loop)
...
# print(customers)
for amount in transactions_clean:
  sales.append(amount[1])
# print(sales)
for thread in transactions_clean:
  thread_sold.append(thread[2])
# print(thread_sold)

##Sales / metrics pieces
#define a variable called total_sales and set it equal to 0
total_sales = 0
#remove the $ from sales, and set them equal to floats,add it to total_sales
for i in sales:
...
for t in thread_sold:
  if not '&' in t:								#if does not include & add to list
    thread_sold_split.append(t)
  else:
    for i in t.split('&'):				#if it does, split at & and add each to list
...
#look into using the date as a sort then to count the colors / sum based on date
print("Today Thread Shed sold: ")
for color in colors:
